package com.example.botcampConfiguration;
 ...
 * 
 * 
 * http. authorizeRequests() .antMatchers("/").permitAll()// accès pour tous
 ...
 * .authenticated().and().csrf().disable().formLogin() // l'accès de fait via un
 * formulaire
 * 
 ...
 * .usernameParameter("email") // paramètres d'authentifications login et
 * password .passwordParameter("password") .and().logout()
 * .logoutRequestMatcher(new AntPathRequestMatcher("/logout")) // route de
 * deconnexion ici /logut .logoutSuccessUrl("/").and().exceptionHandling(); //
 * une fois deconnecté redirection vers login
 * 
 ...
 * .ignoring() .antMatchers("/resources/**","/front/**","/back/**",
